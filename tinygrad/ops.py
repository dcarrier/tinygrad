...
def identity_element(op: BinaryOps, dt: DType):
  return dtypes.as_const({BinaryOps.ADD: 0, BinaryOps.MUL: 1, BinaryOps.MAX: dtypes.min(dt)}[op], dt)
...
class UOp:
  ...
  def commutative(self) -> bool:
    return self.op is UOps.ALU and self.arg in {
      BinaryOps.ADD,
      BinaryOps.MUL,
      BinaryOps.MAX,
      BinaryOps.CMPNE,
  ...
  def cmp_tuple(self) -> Tuple[int, Any, Optional[DType], Tuple[UOp, ...]]:
    ...
    return (
      self.op.value,
      (self.arg if self.op is not UOps.DEFINE_VAR else self.arg.expr) if self.op is not UOps.ALU else self.arg.value,
      self.dtype,
      self.src,
    )
  ...
  def __repr__(self):
    return pretty_print(self, lambda x: f"{type(self).__name__}({x.op}, {x.dtype}, arg={x.argstr()}, src=(%s))")
  ...
  def sink(self, *srcs):
    return UOp(UOps.SINK, None, (self,) + srcs)

  def ufix(self, x):
    return self.const(x) if not isinstance(x, UOp) else x

  def cast(self, dtype=None):
    return type(self)(UOps.CAST, dtype, (self,))

  def bitcast(self, dtype=None):
    return type(self)(UOps.BITCAST, dtype, (self,))

  def gep(self, i: int):
    return type(self)(UOps.GEP, self.dtype.scalar() if self.dtype is not None else None, (self,), i)

  def __neg__(self):
    return self * (-1) if self.dtype != dtypes.bool else self.ne(True)

  def __add__(self, x):
    return self.alu(BinaryOps.ADD, self.ufix(x))
  ...
  def __mul__(self, x):
    return self.alu(BinaryOps.MUL, self.ufix(x))

  def __rmul__(self, x):
    return self.ufix(x).alu(BinaryOps.MUL, self)

  def __floordiv__(self, x):
    return self.alu(BinaryOps.IDIV, self.ufix(x))

  def __truediv__(self, x):
    return self.alu(BinaryOps.MUL, self.ufix(x).alu(UnaryOps.RECIP))

  def __mod__(self, x):
    return self.alu(BinaryOps.MOD, self.ufix(x))

  def __xor__(self, x):
    return self.alu(BinaryOps.XOR, self.ufix(x))

  def __and__(self, x):
    return self.alu(BinaryOps.AND, self.ufix(x))

  def __or__(self, x):
    return self.alu(BinaryOps.OR, self.ufix(x))

  def ne(self, x):
    return self.alu(BinaryOps.CMPNE, self.ufix(x))

  def eq(self, x):
    return -self.ne(x)

  def lt(self, x):
    return self.alu(BinaryOps.CMPLT, self.ufix(x))

  def ge(self, x):
    return (-self).lt(-x + 1)

  def max(self, x):
    return self.alu(BinaryOps.MAX, x)

  def min(self, x):
    return -(-self).max(-x)

  def where(self, x, y):
    return self.alu(TernaryOps.WHERE, x, y)

  def recip(self):
    return self.alu(UnaryOps.RECIP)

  def const(self: Union[UOp, DType, None], b: ConstType | Variable):
    return UOp._const(self.dtype if isinstance(self, UOp) else self, b)
  ...
  def _const(dtype: Optional[DType], b: ConstType | Variable):
    ...
    if isinstance(b, Variable):
      return UOp(UOps.DEFINE_VAR, dtype, (UOp.const(dtypes.int, b.min), UOp.const(dtypes.int, cast(int, b.max))), b)
  ...
  @staticmethod
  def load(*src: UOp, dtype: Optional[DType] = None, **kwargs):
    return type(src[0])(UOps.LOAD, dtype, tuple(src) + tuple(kwargs.values()))

  @staticmethod
  def store(*src: UOp, **kwargs):
    return type((src := (*src, *kwargs.values()))[0])(UOps.STORE, None, src)

  @functools.cached_property
  def parents(self) -> Dict[UOp, None]:
    return {**{x: None for x in self.src}, **{k: None for x in self.src for k in x.parents.keys()}}

  @property  # parents with self
  def sparents(self) -> Dict[UOp, None]:
    return {**self.parents, self: None}

  @functools.cached_property
  def full_shape(self) -> Tuple[sint, ...]:
    if self.op is UOps.SHAPETRACKER:
  ...
  def vars(self) -> Set[UOp]:
    return set([x for x in self.sparents if x.op is UOps.DEFINE_VAR])
  ...
  def const_factor(self) -> int:
    ...
    if self.op is UOps.ALU:
      if self.arg is BinaryOps.ADD:
        return math.gcd(self.src[0].const_factor(), self.src[0].const_factor())
  ...
  def divides(self, v) -> Optional[UOp]:
    if v == 1:
      return self
    if self.op is UOps.CONST:
      return self.const(self.arg // v) if self.arg % v == 0 else None
    if self.op is UOps.ALU:
      if self.arg is BinaryOps.ADD:
  ...
  @property
  def vmin(self) -> UOp:
    return x if (x := self._min_max[0]) is not None and not math.isnan(x.arg) else self.sconst(dtypes.min(cast(DType, self.dtype)))

  @property
  def vmax(self) -> UOp:
    return x if (x := self._min_max[1]) is not None and not math.isnan(x.arg) else self.sconst(dtypes.max(cast(DType, self.dtype)))

  @functools.cached_property
  def _min_max(self) -> Tuple[Optional[UOp], Optional[UOp]]:
    # NOTE: returned UOp is assumed to be CONST
    if self.op is UOps.DEFINE_VAR and self.src:
      return self.src[0], self.src[1] if isinstance(self.src[1].arg, int) else None
    if self.op is UOps.RANGE:
      return self.src[0].vmin, (self.src[1] - 1).vmax
    # TODO: UOps.SPECIAL is UOps.DEFINE_VAR
    if self.op is UOps.SPECIAL:
      return self.const(0), self.const(self.arg[1] - 1) if isinstance(self.arg[1], int) else None
    if self.op is UOps.CONST:
      return self, self
    if self.op is UOps.ALU and cast(DType, self.dtype).count == 1:
      ...
      if self.arg is BinaryOps.ADD:
        return self.sconst(s0.vmin.arg + s1.vmin.arg), self.sconst(s0.vmax.arg + s1.vmax.arg)
      ...
      if self.arg is BinaryOps.MOD and s1.vmin.arg > 0:
        return self.sconst(0), self.sconst(s1.vmax.arg - 1)
      if self.arg is BinaryOps.IDIV and s1.op is UOps.CONST:
        if s1.arg > 0:
          return self.sconst(s0.vmin.arg // s1.arg), self.sconst(s0.vmax.arg // s1.arg)
        if s1.arg < 0:
          return self.sconst(-(s0.vmax.arg // -s1.arg)), self.sconst(-(s0.vmin.arg // -s1.arg))
      if self.arg is BinaryOps.MAX:
        return self.sconst(max(s0.vmin.arg, s1.vmin.arg)), self.sconst(max(s0.vmax.arg, s1.vmax.arg))
      if self.arg is BinaryOps.CMPLT:
        return (UOp.sconst(dtypes.bool, s0.vmax.arg < s1.vmin.arg), UOp.sconst(dtypes.bool, s0.vmin.arg < s1.vmax.arg))
    return None, None
...
def get_location() -> Tuple[str, int]:
  ...
  while (frm.f_code.co_filename.endswith("/ops.py") or frm.f_code.co_filename == "<string>") and frm.f_back is not None:
    frm = frm.f_back
...
class NOp(UOp):
  ...
  @staticmethod
  @functools.lru_cache(None)
  def var(name: Optional[str] = None, dtype: Optional[DType] = None):
    return NOp(UOps.NOOP, dtype=dtype, name=name)

  @staticmethod
  @functools.lru_cache(None)
  def cvar(name: Optional[str] = None, dtype: Optional[DType] = None):
    return NOp(UOps.CONST, dtype=dtype, name=name)
  ...
  def upat(self: NOp) -> UPat:
    return (
      UPat(name=self.name, dtype=self.dtype, location=self.location)
      if self.op is UOps.NOOP
      else UPat(
        self.op,
        self.arg,
        (list if self.commutative() else tuple)([src.upat for src in self.src]) or None,
        self.name,
        self.dtype,
        self.allow_any_len,
...
class UPat:
  def __init__(
    ...
    if isinstance(src, list):
      self.src = list(itertools.permutations(src)) if not all_same(src) else [src]
  ...
  def __repr__(self):
    def rep(x):
      ...
      return form % (
        None if x.op is None else ("(%s)" % ", ".join(map(str, x.op))),
        x.arg,
        repr(x.name),
        set(x.dtype) if x.dtype else None,
        x.allowed_len == 0,
        "[%s]" if x.src and len(x.src) > 1 else "(%s)",
      )

    return pretty_print(
      self, rep, srcfn=lambda x: None if x.src is None else [next(x.src[0])] if isinstance(x.src[0], itertools.repeat) else x.src[0]
    )


def _match(uop: UOp, pat: UPat, store: Dict[str, UOp]) -> List[Dict[str, UOp]]:
  if (
    (pat.name is not None and store.setdefault(pat.name, uop) is not uop)
    or (pat.dtype is not None and uop.dtype not in pat.dtype)
    or (pat.arg is not None and pat.arg != uop.arg)
    or (pat.op is not None and uop.op not in pat.op)
    or (pat.allowed_len != 0 and len(uop.src) != pat.allowed_len)
  ):
    return []
  if pat.src is None:
    return [store]
  ...
  for vp in pat.src:
    ...
    for uu, vv in zip(uop.src, vp):
      new_stores = [rstore for nstore in new_stores for rstore in _match(uu, vv, nstore)]
...
class PatternMatcher:
  def __init__(self, patterns: Sequence[Tuple[Union[UPat, NOp], Callable]]):
    ...
    for p, fxn in self.patterns:
      ...
      for uop in p.op:
        self.pdict[(uop, p.arg)].append((p, fxn, p.early_reject))

  @functools.lru_cache(None)  # pylint: disable=method-cache-max-size-none
  def __add__(self, more: PatternMatcher):
    ...
    for p, fxn, early_reject in itertools.chain(self.pdict[(uop.op, uop.arg)], self.pdict[(uop.op, None)]):
      ...
      if (matches := _match(uop, p, {})) and (ret := fxn(**matches[0])) is not None:
        ...
        if TRACK_MATCH_STATS >= 2:
          print(f"{et * 1e6:7.2f} us -- ", p.printable())
        return ret  # NOTE: if it returns None, we keep trying to match
      match_stats[p][2] += time.perf_counter() - st
    return None
...
if TRACK_MATCH_STATS:
  ...
  def print_match_stats():
    ...
    for k, v in sorted(list(match_stats.items()), key=lambda x: x[1][2]):
      ...
      print(f"{v[0]:6d} / {v[1]:7d} -- {v[3] * 1000.0:9.2f} / {v[2] * 1000.0:9.2f} ms -- {loc_str:15s}", k.printable())
      ret = [x + y for x, y in zip(ret, v)]
    print(f"{ret[0]:6d} / {ret[1]:7d} -- {ret[3] * 1000.0:9.2f} / {ret[2] * 1000.0:9.2f} ms -- TOTAL")
...
python_alu: Dict[Op, Callable] = {
  ...
  BinaryOps.SHR: operator.rshift,
  BinaryOps.SHL: operator.lshift,
  BinaryOps.MUL: operator.mul,
  BinaryOps.ADD: operator.add,
  BinaryOps.XOR: operator.xor,
  BinaryOps.MAX: max,
  BinaryOps.CMPNE: operator.ne,
  BinaryOps.CMPLT: operator.lt,
  BinaryOps.OR: operator.or_,
  BinaryOps.AND: operator.and_,
...
truncate: Dict[DType, Callable] = {
  ...
  dtypes.int32: lambda x: ctypes.c_int32(x).value if isinstance(x, int) else x,
...
def uop_alu_resolve(u: UOp) -> sint:
  if u.op in {UOps.CONST, UOps.DEFINE_VAR}:
    return u.arg
  if u.op is UOps.ALU:
    return exec_alu(u.arg, cast(DType, u.dtype), tuple(map(uop_alu_resolve, u.src)))
...
def type_verify(uops):
  for u in uops:
    ...
    if uop is UOps.DEFINE_LOCAL:
      assert isinstance(dtype, PtrDType), f"invalid dtype for local buffer {dtype}"
    if uop is UOps.DEFINE_GLOBAL:
      assert isinstance(dtype, (PtrDType, ImageDType)), f"invalid dtype for global buffer {dtype}"
    if isinstance(dtype, ImageDType):
      assert uop is UOps.DEFINE_GLOBAL, f"{uop} can't be image"
    if uop is UOps.SHAPETRACKER:
      assert len(src) == 0, f"SHAPETRACKER must only define a ShapeTracker arg {uop}"
    if uop is UOps.REDUCE_AXIS:
      assert isinstance(arg, tuple) and len(arg) == 2 and arg[0] in BinaryOps, f"invalid arg for REDUCE_AXIS {arg}"
    ...
    if uop in {UOps.CAST, UOps.BITCAST, UOps.VECTORIZE}:
      assert arg is None and dtype is not None  # type is the output type, not an arg
    if uop is UOps.CAST:
      assert dtype.count == 1 and len(src) == 1
    ...
    if uop is UOps.LOAD and len(src) > 3 and src[3].op is UOps.ALU:
      assert src[3].dtype == dtypes.bool and src[2].dtype == dtype
    if uop is UOps.GEP:
      assert dtype == src[0].dtype.scalar(), f"GEP of {src[0].dtype=} should be {src[0].dtype.scalar()} != {dtype}"
    if uop is UOps.STORE:
      assert dtype is None, f"{uop} dtype must be None, got {dtype}"
      if len(src) == 4:
        assert src[3].dtype == dtypes.bool, f"gate dtype mismatch {src[3].dtype} != {dtypes.bool}"
    if uop is UOps.ALU:
      if arg in UnaryOps:
  ...
  if ignore_indexing:
    for u in uops:
      if u.op is UOps.LOAD:
        ...
        if len(u.src) > 3:
          dont_count = dont_count.union(u.src[2].sparents)
      elif u.op is UOps.STORE:
        dont_count = dont_count.union(u.src[1].sparents)
        if len(u.src) > 3:
          dont_count = dont_count.union(u.src[3].sparents)
