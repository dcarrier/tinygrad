...
class Log(Function):
  ...
  def backward(self, grad_output: LazyBuffer) -> LazyBuffer:
    return grad_output.e(BinaryOps.MUL, self.x.e(UnaryOps.RECIP))
...
class BitwiseAnd(Function):
  def forward(self, x: LazyBuffer, y: LazyBuffer) -> LazyBuffer:
    return x.e(BinaryOps.AND, y)


class BitwiseOr(Function):
  def forward(self, x: LazyBuffer, y: LazyBuffer) -> LazyBuffer:
    return x.e(BinaryOps.OR, y)


class Threefry(Function):
  def forward(self, x: LazyBuffer, seed: LazyBuffer) -> LazyBuffer:
    return x.e(BinaryOps.THREEFRY, seed)
...
class Mul(Function):
  ...
  def backward(self, grad_output: LazyBuffer) -> Tuple[Optional[LazyBuffer], Optional[LazyBuffer]]:
    return self.y.e(BinaryOps.MUL, grad_output) if self.needs_input_grad[0] else None, self.x.e(BinaryOps.MUL, grad_output) if self.needs_input_grad[
      1
    ] else None
